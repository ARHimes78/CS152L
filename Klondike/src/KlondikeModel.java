...
public class KlondikeModel {
    ...
    public void drawNextCard() {
        if (deck.size() > 0) {
            deck.move(drawPile);
            drawPile.getTop().setFaceUp(true);
        } else {
            while(drawPile.size() > 0) {
                drawPile.move(deck);
                deck.getTop().setFaceUp(false);
            }
        }
    }
    ...
    public void klondikeMove(Deck here, Deck there) {
        Card c = there.getTop(); // The card of top of that, if any
        ...
        if ((foundationIndex >= 0) && (foundationIndex < 4)) {
            ...
            if (c != null && (c.getSuit().ordinal() == foundationIndex)
                && (((foundation.size() == 0) && (c.getRank() == Rank.ACE)) 
                    || ((foundation.size() > 0)
                        && (c.getRank().ordinal() == foundation.getTop().getRank().ordinal() + 1)))) {
                source.move(foundation);
                if (source.size() > 0) {
                    source.getTop().setFaceUp(true);
                }
            }
        }
    }
    ...
    public void moveToTableau(Deck source, int tableauIndex) {
        ...
        if (source == drawPile) {
            Card sourceCard = source.getTop();
            Card destCard = destination.getTop();
            if (sourceCard != null &&
