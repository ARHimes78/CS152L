public class LibraryTest {
    ...
    public static void main(String[] args) {
        ...
        Book dupBook = new Book( "To The Lighthouse", new Author("Woolf","Virginia") );
        dupBook.setISBN( "9781907523588" );
        ...
        if( library.checkOutBook( b1 ).equals( "Checked out!" )  ) { tScore+=1; }
        System.out.println( " - checkOutBook works when a book can be checked out: " + tScore + "/1" );
        score += tScore; tScore = 0;  
        if( library.checkOutBook( b1 ).equals( "All out of copies." )  ) { tScore+=1; }
        System.out.println( " - checkOutBook works when a book is out of copies: " + tScore + "/1" );
        score += tScore; tScore = 0;  
        if( library.checkOutBook( b3 ).equals( "Book not found." ) ) { tScore+=1; }
        System.out.println( " - checkOutBook works when a book is not in the library: " + tScore + "/1" );
        score += tScore; tScore = 0;
        if( library.statusString().equals( "Total unique books: 31\n"+
                                           "Total number of copies: 45\n"+
        ...
        if( library.checkInBook( b3 ).equals( "Book not found." ) ) { tScore+=1; }
        System.out.println( " - checkInBook works when a book is not in the library: " + tScore + "/1" );
        score += tScore; tScore = 0;
        if( library.checkInBook( b2 ).equals( "All of our copies are already checked in." )  ) { tScore+=1; }
        System.out.println( " - checkInBook works when a book is not checked out: " + tScore + "/1" );
        score += tScore; tScore = 0;  
        if( library.checkInBook( b1 ).equals( "Checked in!" )  ) { tScore+=1; }
        System.out.println( " - checkInBook works when a book is checked back in: " + tScore + "/1" );
        score += tScore; tScore = 0;  
        if( library.statusString().equals( "Total unique books: 31\n"+
                                           "Total number of copies: 45\n"+
        ...
        library.checkOutBook( b1 );
        library.checkOutBook( b2 );
        library.checkOutBook( libraryBooks[25] );
        library.checkOutBook( libraryBooks[25] );
        ...
        if( library.listBooksByAuthor( new Author( "Eggers","Dave" )).equals("The Circle. Eggers, Dave.\n") ) {
            tScore++;
        }
        if( library.listBooksByAuthor( new Author( "Adler","Renata" )).equals("Pitch Dark. Adler, Renata.\n"+
                                                                              "Speedboat. Adler, Renata.\n") ) {
            tScore++;
        }
        if( library.listBooksByAuthor( new Author( "Ballard","J.G." )).equals("No books by Ballard, J.G..") ) {
            tScore++;
        }
        if( library.listBooksByAuthor( new Author( "Joyce", "James" )).equals("Ulysses. Joyce, James.\n"+
        ...
        library.checkOutBook( libraryBooks[1] );
        if( library.toString().equals("0. Ulysses. Joyce, James. : 1/1\n"+
